...
namespace Cloudents.Query.Tutor
{
    public class TutorListQuery : IQuery<ListWithCountDto<TutorCardDto>>
    {
        ...
        internal sealed class TutorListQueryHandler : IQueryHandler<TutorListQuery, ListWithCountDto<TutorCardDto>>
        {
            ...
            {
                const string sql = @"Select 'Tutor' as 'Type', rt.Id as UserId, rt.Name as 'Name', rt.Image as 'Image', rt.Courses, rt.Subjects, rt.Price,
...
CASE
   ...
   WHEN exists (
				...
				where rt.Id = uc.UserId and uc.CanTeach = 1 --and SubjectId is not null
   intersect
				select c2.SubjectId from sb.UsersCourses uc2 join sb.Course c2 on uc2.CourseId = c2.Name 
				where uc2.UserId = @userid --and SubjectId is not null
				) THEN 1
   else 0
