...
namespace Zbang.Cloudents.MobileApp2.Controllers
{
    ...
    public class SearchController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Items(string term, int page, int sizePerPage = 20)
        {
            ...
            var query = new SearchItemsQuery(term, User.GetCloudentsUserId(), universityId.Value, page, sizePerPage);
            var retVal = await ItemSearchService2.SearchItemOldMobileServiceAsync(query, default(CancellationToken)) ?? new List<SearchItems>();
            return Request.CreateResponse(retVal.Select(s => new
            {
                s.Name,
                s.Id,
        ...
        public async Task<HttpResponseMessage> Items2(string term, int page, int sizePerPage = 20)
        {
            ...
            var retVal = await ItemSearchService2.SearchItemAsync(query, default(CancellationToken)) ?? new List<SearchItems>();
            return Request.CreateResponse(retVal.Select(s => new
            {
                s.Name,
                s.BoxId,
