...
export default {
    activateFunction :{
        ...
        note({ source,university,course,term,page,sort}) {
            return new Promise((resolve, reject) => {
                search.getDocument({ source, university, course, term, page, sort }).then(({ body }) => resolve({ source: body.facet, data: body.result.map(val => { return { ...val, template: "item" } }) }))
            })
        },
