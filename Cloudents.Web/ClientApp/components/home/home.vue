...
<template v-once>
    <page-layout :links="links" :submitFunction="search" :items="items" :sites="sites" :selectosFunction="selectos"
        ...
        <v-text-field slot="inputField" solo @keyup.enter="search" autocomplete="off" type="text" required name="q" :class="{'record':isRecording}" id="transcript" v-model.trim="msg" :placeholder="placeholder" prepend-icon="sbf-search" :append-icon="voiceEnable?(isRecording?'sbf-mic-recording':'sbf-mic'):''" :append-icon-cb="$_voiceDetection"></v-text-field>
        ...
        <features-section slot="featuresSection" :features="features">
            <component class="item" slot="item" :is="props.type" slot-scope="props"></component>
        </features-section>
