<template v-once>
    <a class="d-block job-cell" :target="($vuetify.breakpoint.xsOnly)?'_self':'_blank'" :href="item.url">
       <span @click="$ga.event('Search_Results', 'job',`#${index+1}_${item.source}`)"> <div
        ...
        <v-layout row wrap class="properties">
            <v-flex class="property" sm4 xs6><company-icon class="mr-2"></company-icon><span class="company"
                                                                                             :title="item.company">{{item.company?item.company:'N/A'}}</span></v-flex>
            <v-flex class="property" sm4 xs6><location-icon class="mr-2"></location-icon><span class="location"
                                                                                               v-if="item.address">{{item.address}}</span></v-flex>
            <v-flex sm4 xs0></v-flex>
            <v-flex class="property mt-3" sm4 xs6><paid-icon
                    class="mr-2"></paid-icon><span>{{item.compensationType}}</span></v-flex>
