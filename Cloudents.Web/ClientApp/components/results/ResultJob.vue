<template v-once>
    <a class="d-block job-cell" :target="($vuetify.breakpoint.xsOnly)?'_self':'_blank'" :href="item.url" @click="goog_report_conversion(item.url)">
       <span @click="trackOutBound(()=>$ga.event('Search_Results', 'job',`#${index+1}_${item.source}`))"> <div class="cell-title-wrap mb-1"><span class="cell-title">{{item.title}}</span></div>
        <div class="desc">
