                        ...
                        <v-layout column>
                            ...
                            <slot name="resultData" :items="items">
                                ...
                                <v-flex class="result-cell mb-3" xs-12 v-for="(item,index) in items" :key="index" :class="(index>6?'order-xs6': index>2 ? 'order-xs3' : 'order-xs2')">
                                        ...
                                        <div>
                                            <span class="question-viewer" v-if="!item.hasCorrectAnswer && name==='ask' && item.watchingNow === 1 && item.watchingNow !== 0" :style="watchinNowStyle(item)" >{{item.watchingNow}} <span v-language:inner>result_user_answering</span></span>
                                            <span class="question-viewer" v-if="!item.hasCorrectAnswer && name==='ask' && item.watchingNow !== 1 && item.watchingNow !== 0" :style="watchinNowStyle(item)" >{{item.watchingNow}} <span v-language:inner>result_users_answering</span></span>
                                            <div class="show-btn" v-if="accountUser && item &&  item.user && accountUser.id !== item.user.id || name!=='ask'" :class="'color-'+$route.path.slice(1)" v-language:inner>{{name==='ask' && !item.hasCorrectAnswer?'result_answer':'result_showme'}}</div>
                                            <div class="show-btn" v-if="!accountUser && item && item.user || name!=='ask'" :class="'color-'+$route.path.slice(1)" v-language:inner>{{name==='ask'?'result_answer':'result_showme'}}</div>
                                        </div>
