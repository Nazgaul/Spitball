        ...
        <v-expansion-panel-content v-for="(k,index) in props.values" :key="k.modelId" hide-actions v-bind:value="true">
            ...
            <div class="sort-filter pa-2">
                <div v-for="s in k.data" :key="(s.id?s.id:s)" class="filter">
                    <input type="checkbox" :id="`filter-option${s.id}`" :checked="props.checkesVals.includes(s.id?s.id:s.toString())" @change="props.callback({id:k.modelId,val:(s.id?s.id:s),type:$event})"/>
                    <label :for="`filter-option${s.id}`">{{s.name?s.name:s}}</label>
                </div>
