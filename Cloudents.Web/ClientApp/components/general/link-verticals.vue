<template>
    <v-list subheader>
        <template v-for="(list,index) in verticals">
            <v-subheader class="ml-2">{{list.label}}</v-subheader>
            <v-list-tile append="vertical.name" v-for="vertical in list.data" :key="vertical.name" replace="true" :to="{name:vertical.id}">
