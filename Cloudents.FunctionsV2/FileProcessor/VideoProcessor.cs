...
namespace Cloudents.FunctionsV2.FileProcessor
{
    public class VideoProcessor : IFileProcessor
    {
        ...
        public async Task ProcessFile(long id, CloudBlockBlob blob, ILogger log, CancellationToken token)
        {
        ...
        public async Task MoveImage(long id, IBinder binder, CancellationToken token)
        {
            ...
            var destinationBlob = await binder.BindAsync<CloudBlockBlob>(new BlobAttribute($"{StorageContainer.File.Name}/{StorageContainer.File.RelativePath}/{id}/preview-0.jpg"), token);
        ...
        public Task CreateLocator(long id, CancellationToken token)
        {
            //var id = RegEx.NumberExtractor.Match(assetName).Value;
            return _videoService.CreateShortStreamingLocator(id, token);
        }

        public async Task UpdateDuration(long id, IBinder binder, CancellationToken token)
        {
            //var id = RegEx.NumberExtractor.Match(assetName).Value;
