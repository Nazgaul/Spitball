  ...
  <div class="cashout-table-container">
    ...
    <v-layout>
      ...
      <v-flex xs3>
        <v-select
          ...
          label="state"
          v-model="state"
          @change="getCourseList2( state, search)"
        ></v-select>
      </v-flex>
      ...
      <v-flex xs4 sm4 md4 offset-xs2>
        <v-text-field
          ...
          single-line
          hide-details
          @keyup.enter.native="getCourseList2( state, search)"
        ></v-text-field>
      </v-flex>
    </v-layout>
    <v-data-table
    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td>
          <span>
            <v-icon small color="green" class="mr-2" @click="editItem(props.item)">call_to_action</v-icon>
          </span>
        </td>
      </template>
    ...
    <v-dialog persistent v-model="dialog" max-width="500px">
      <v-card>
        <v-card-title>
          <span
            v-show="radios === 'merge' || radios === 'rename'"
            class="headline"
        ...
        <v-card-actions>
          ...
          <v-btn color="red darken-1" :disabled="disableDoneBtn" flat @click="done()">Done</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
  </div>
...
export default {
  ...
  computed: {
    disableDoneBtn() {
      return (
        (this.radios === "merge" && !this.picked) ||
        (this.radios === "rename" && this.newName.length < 4)
      );
    }
  },
  watch: {
  ...
  methods: {
    ...
    editItem(item) {
      ...
      if (this.radios === "merge") {
        this.courseMigrate({
          newCourse: this.editedItem.name,
          oldCourse: this.picked.name
        });
    ...
    courseMigrate(item) {
      ...
      migrateCourses(item.newCourse, item.oldCourse).then(
        resp => {
          console.log("got migration resp success");

          this.$toaster.success(
            `Course ${item.newCourse} merged into ${item.oldCourse}`
          );
    ...
    rename(course, newName) {
      ...
      rename(course.name, newName).then(
        resp => {
      ...
      deleteCourse(item).then(
        ...
        error => {
          if (error.response.data == `constraint Violation`) {
            this.$toaster.error(
              `This course has document/question to it. Need to merge it with another course`
            );
          } else {
    ...
    getCourseList2(state, filter) {
      ...
      var self = this;
      getCourseList(state, filter).then(
        list => {
