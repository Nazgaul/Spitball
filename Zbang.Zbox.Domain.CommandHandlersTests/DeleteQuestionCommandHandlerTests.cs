...
namespace Zbang.Zbox.Domain.CommandHandlersTests
{
    ...
    public class DeleteQuestionCommandHandlerTests
    {
        ...
        public void Handle_DeleteQuestionWithPermission_DeleteQuestion()
        {
            ...
            var user = new User("some email", " some small image", "some largeImage", "some first name", "some middle name", "some last name", "en-US");
            user.GetType().GetProperty("Id").SetValue(user, userId);
            var ownerBox = new User("some email1", " some small image1", "some largeImage1", "some first name1", "some middle name1", "some last name1", "en-US");
