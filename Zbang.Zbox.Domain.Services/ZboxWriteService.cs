    ...
    public class ZboxWriteService : IZboxWriteService, IZboxServiceBootStrapper, Autofac.IStartable
    {
        ...
        public async Task<LikeCommentCommandResult> LikeCommentAsync(LikeCommentCommand command)
        {
            using (UnitOfWork.Start())
            {
                ...
                await Task.WhenAll(t2, t1).ConfigureAwait(true);
                var result = await m_CommandBus.DispatchAsync<LikeCommentCommand, LikeCommentCommandResult>(command).ConfigureAwait(true);
